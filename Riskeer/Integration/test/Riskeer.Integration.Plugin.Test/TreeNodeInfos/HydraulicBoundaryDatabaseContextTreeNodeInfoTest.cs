    ...
    public class HydraulicBoundaryDatabaseContextTreeNodeInfoTest : NUnitFormTest
    {
        ...
        public void ChildNodeObjects_HydraulicBoundaryDatabaseLinked_ReturnsChildrenOfData()
        {
            ...
            using (var plugin = new RiskeerPlugin())
            {
                ...
                var hrdFileGroupContext = (HrdFileGroupContext) objects[0];
                Assert.AreSame(assessmentSection.HydraulicBoundaryDatabase, hrdFileGroupContext.WrappedData);
                Assert.AreSame(assessmentSection, hrdFileGroupContext.AssessmentSection);
