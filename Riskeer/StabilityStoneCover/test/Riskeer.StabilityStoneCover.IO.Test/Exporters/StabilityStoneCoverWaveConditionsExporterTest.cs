    ...
    public class StabilityStoneCoverWaveConditionsExporterTest
    {
        ...
        public void Export_ValidDataWithCalculationTypeBoth_ValidFile()
        {
            ...
            using (new DirectoryDisposeHelper(TestHelper.GetScratchPadPath(), folderName))
            {
                ...
                string fileContent = File.ReadAllText(filePath);
                string expectedText = $"Naam berekening; Naam HB locatie; X HB locatie (RD) [m]; Y HB locatie (RD) [m]; Naam voorlandprofiel; Dam gebruikt; Voorlandgeometrie gebruikt; Type bekleding; Doelkans [1/jaar]; Waterstand [m+NAP]; Golfhoogte (Hs) [m]; Golfperiode (Tp) [s]; Golfrichting t.o.v. dijknormaal [°]; Golfrichting t.o.v. Noord [°]; Belastingparameter (S) [-]{Environment.NewLine}" +
                                      $"aCalculation; aLocation; 44.000; 123.456; foreshoreA; nee; nee; Steen (blokken); 1/100; 1.10; 2.20; 3.30; 4.40; 5.50; 6.60{Environment.NewLine}" +
                                      $"aCalculation; aLocation; 44.000; 123.456; foreshoreA; nee; nee; Steen (zuilen); 1/100; 1.10; 2.20; 3.30; 4.40; 5.50; 6.60{Environment.NewLine}";
                Assert.AreEqual(expectedText, fileContent);
            }
        }
