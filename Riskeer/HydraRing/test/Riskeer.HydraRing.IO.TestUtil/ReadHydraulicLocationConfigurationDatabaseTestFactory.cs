...
namespace Riskeer.HydraRing.IO.TestUtil
{
    ...
    public static class ReadHydraulicLocationConfigurationDatabaseTestFactory
    {
        ...
        private static ReadHydraulicLocationConfigurationDatabase Create(IEnumerable<long> hrdLocationIds,
                                                                         ...
                                                                         long trackId)
        {
            return new ReadHydraulicLocationConfigurationDatabase(hrdLocationIds.Select(hrdLocationId => CreateReadHydraulicLocation(hrdLocationId, trackId)).ToList(),
                                                                  settings,
                                                                  new[]
                                                                  {
                                                                      new ReadTrack(trackId, "hrd.sqlite", false)
                                                                  });
        }
