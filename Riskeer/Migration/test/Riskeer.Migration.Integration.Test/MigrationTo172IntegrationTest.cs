...
namespace Riskeer.Migration.Integration.Test
{
    public class MigrationTo172IntegrationTest
    {
                                                                                                            ...
                                                                                                            int lowerLimitReturnPeriod)
        {
            ...
            using (new FileDisposeHelper(targetFilePath))
            {
                File.Copy(TestHelper.GetTestDataPath(TestDataPath.Riskeer.Migration.Core,
        ...
        private static void AssertLogDatabase(string logFilePath)
        {
            using (var reader = new MigrationLogDatabaseReader(logFilePath))
            {
                ...
                MigrationLogTestHelper.AssertMigrationLogMessageEqual(
                    ...
                    messages[i++]);
                MigrationLogTestHelper.AssertMigrationLogMessageEqual(
                    new MigrationLogMessage("17.1", newVersion, "* Alle berekende resultaten zijn verwijderd, behalve die van het faalmechanisme 'Piping' waarbij de waterstand handmatig is ingevuld."),
                    messages[i++]);
