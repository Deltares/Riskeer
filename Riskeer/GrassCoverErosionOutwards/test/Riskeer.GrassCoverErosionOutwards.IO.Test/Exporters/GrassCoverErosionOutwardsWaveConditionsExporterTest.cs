    ...
    public class GrassCoverErosionOutwardsWaveConditionsExporterTest
    {
        ...
        public void Export_ValidDataWithCalculationTypeWaveRunUpAndWaveImpact_ValidFile()
        {
            ...
            using (new DirectoryDisposeHelper(TestHelper.GetScratchPadPath(), folderName))
            {
                ...
                string fileContent = File.ReadAllText(filePath);
                string expectedText = $"Naam berekening; Naam HB locatie; X HB locatie (RD) [m]; Y HB locatie (RD) [m]; Naam voorlandprofiel; Dam gebruikt; Voorlandgeometrie gebruikt; Type bekleding; Doelkans [1/jaar]; Waterstand [m+NAP]; Golfhoogte (Hs) [m]; Golfperiode (Tp) [s]; Golfrichting t.o.v. dijknormaal [°]; Golfrichting t.o.v. Noord [°]; Sterkte [-]{Environment.NewLine}" +
                                      $"aCalculation; aLocation; 44.000; 123.456; foreshoreA; nee; nee; Gras (golfoploop); 1/100; 1.10; 2.20; 3.30; 4.40; 5.50; 6.60{Environment.NewLine}" +
                                      $"aCalculation; aLocation; 44.000; 123.456; foreshoreA; nee; nee; Gras (golfklap); 1/100; 1.10; 2.20; 3.30; 4.40; 5.50; 6.60{Environment.NewLine}";
                Assert.AreEqual(expectedText, fileContent);
            }
        }
        ...
        [Test]
        public void Export_ValidDataWithCalculationTypeWaveImpactWithWaveDirection_ValidFile()
        {
            // Setup
            string folderName = $"{nameof(GrassCoverErosionOutwardsWaveConditionsExporterTest)}.{nameof(Export_ValidDataWithCalculationTypeWaveImpact_ValidFile)}";
            string directoryPath = TestHelper.GetScratchPadPath(folderName);
            using (new DirectoryDisposeHelper(TestHelper.GetScratchPadPath(), folderName))
            {
                ...
                string fileContent = File.ReadAllText(filePath);
                string expectedText = $"Naam berekening; Naam HB locatie; X HB locatie (RD) [m]; Y HB locatie (RD) [m]; Naam voorlandprofiel; Dam gebruikt; Voorlandgeometrie gebruikt; Type bekleding; Doelkans [1/jaar]; Waterstand [m+NAP]; Golfhoogte (Hs) [m]; Golfperiode (Tp) [s]; Golfrichting t.o.v. dijknormaal [°]; Golfrichting t.o.v. Noord [°]; Sterkte [-]{Environment.NewLine}" +
                                      $"aCalculation; aLocation; 44.000; 123.456; foreshoreA; nee; nee; Gras (golfklap met golfrichting); 1/100; 1.10; 2.20; 3.30; 4.40; 5.50; 6.60{Environment.NewLine}";
                Assert.AreEqual(expectedText, fileContent);
            }
        }

        [Test]
        public void Export_ValidDataWithCalculationTypeWaveRunUpAndWaveImpactWithWaveDirection_ValidFile()
        {
            // Setup
            string folderName = $"{nameof(GrassCoverErosionOutwardsWaveConditionsExporterTest)}.{nameof(Export_ValidDataWithCalculationTypeWaveImpact_ValidFile)}";
            string directoryPath = TestHelper.GetScratchPadPath(folderName);
