    ...
    public class WmsServer
    {
        ...
        public static void ParseQueryString(Map map, Capabilities.WmsServiceDescription description)
        {
            ...
            else if (String.Compare(context.Request.Params["REQUEST"], "GetMap", ignorecase) == 0) //Map requested
            {
                ...
                {
                    WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue, "Required parameter WIDTH not specified");
                    return;
                }
                if (context.Request.Params["HEIGHT"] == null)
                {
                    WmsException.ThrowWmsException(WmsException.WmsExceptionCode.InvalidDimensionValue, "Required parameter HEIGHT not specified");
                    return;
                }
        ...
        private static IEnvelope ParseBBOX(string strBBOX)
        {
            ...
            double maxx = 0;
            double maxy = 0;
            if (!double.TryParse(strVals[0], NumberStyles.Float, Map.numberFormat_EnUS, out minx))
            {
                return null;
            }
            if (!double.TryParse(strVals[2], NumberStyles.Float, Map.numberFormat_EnUS, out maxx))
            {
                return null;
            }
            if (maxx < minx)
            ...
            if (!double.TryParse(strVals[1], NumberStyles.Float, Map.numberFormat_EnUS, out miny))
            {
                return null;
            }
            if (!double.TryParse(strVals[3], NumberStyles.Float, Map.numberFormat_EnUS, out maxy))
            {
                return null;
            }
            if (maxy < miny)
