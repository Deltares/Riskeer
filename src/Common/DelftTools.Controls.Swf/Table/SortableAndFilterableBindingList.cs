...
namespace DelftTools.Controls.Swf.Table
{
    public class SortableAndFilterableBindingList<T> : BindingList<T>, IBindingListView where T : class
    {
        ...
        private List<FilterOperation> CreateFilterLookUp()
        {
            ...
            if (string.IsNullOrEmpty(filter)) return lookUp;

            var propertyFilters = filter.Split(new[] { " AND " }, StringSplitOptions.RemoveEmptyEntries);
        ...
        private class FilterOperation
        {
            ...
            {
                if (filterOperation == "=") return FilterOption.Equal;
                if (filterOperation == "<>") return FilterOption.NotEqual;
                if (filterOperation == "<") return FilterOption.SmallerThen;
                if (filterOperation == ">") return FilterOption.GreaterThen;
                if (filterOperation == "Like")
                {
                    var value = filterValue.ToString();
                    if (value.StartsWith("%") && value.EndsWith("%")) return FilterOption.Contains;
                    if (value.StartsWith("%")) return FilterOption.EndsWith;
                    if (value.EndsWith("%")) return FilterOption.StartsWith;
                }
