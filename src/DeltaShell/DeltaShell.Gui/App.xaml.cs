...
namespace DeltaShell.Gui
{
    ...
    public partial class App : Application
    {
        ...
        static int waitForProcessId = -1; // start Delta Shell after this process will exit (used during restart)
        ...
        public static void RunDeltaShell(IMainWindow mainWindow)
        {
            ...
            var loaderDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            ...
            gui = new DeltaShellGui { MainWindow = mainWindow };
        ...
        private static void HandleExceptionOnMainThread(Exception exception, bool isTerminating)
        {
            ...
            if (control != null && control.InvokeRequired)
            {
                ...
                control.Invoke(new ExceptionDelegate(HandleException), new object[] { exception, isTerminating });
            }
            else
            {
        ...
        private static void ParseArguments(IEnumerable<string> arguments)
        {
            var p = new OptionSet
                {
                    { "<>", delegate(string projectPath) { if (Path.GetExtension(projectPath) == ".dsproj") { projectFilePath = projectPath; } }},
