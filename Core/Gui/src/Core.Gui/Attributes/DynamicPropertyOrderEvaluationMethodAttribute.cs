...
namespace Core.Gui.Attributes
{
    ...
    public sealed class DynamicPropertyOrderEvaluationMethodAttribute : Attribute
    {
        ...
        private static void ValidateMethodInfo(MethodInfo methodInfo)
        {
            if (methodInfo.ReturnType != typeof(int))
            {
                string message = string.Format(CultureInfo.CurrentCulture,
                                               Resources.DynamicPropertyOrderEvaluationMethod_must_return_int_on_Class_0_,
            ...
            {
                string message = string.Format(CultureInfo.CurrentCulture,
                                               Resources.DynamicPropertyOrderEvaluationMethod_must_have_string_argument_on_Class_0_,
                                               methodInfo.DeclaringType);
                throw new MissingMethodException(message);
            }
        }
        ...
        private static MethodInfo GetPropertyOrderMethod(object obj)
        {
            ...
            {
                string message = string.Format(CultureInfo.CurrentCulture,
                                               Resources.DynamicPropertyOrderEvaluationMethod_not_found_or_not_public_on_Class_0_,
                                               obj.GetType());
                throw new MissingMethodException(message);
            }
            ...
            {
                string message = string.Format(CultureInfo.CurrentCulture,
                                               Resources.DynamicPropertyOrderEvaluationMethod_only_one_allowed_per_Class_0_,
                                               obj.GetType());
                throw new MissingMethodException(message);
            }
