...
namespace Core.Common.Gui.Test.Commands
{
    ...
    {
        [Test]
        public void GetDataOfActiveView_NoActiveView_ReturnNull()
        {
            // Setup
            var mocks = new MockRepository();
            var viewsList = mocks.Stub<IViewList>();
            viewsList.ActiveView = null;
            var documentViewController = mocks.Stub<IDocumentViewController>();
            documentViewController.Stub(c => c.DocumentViews).Return(viewsList);
            ...
            // Call
            var viewData = commandHandler.GetDataOfActiveView();

            // Assert
            Assert.IsNull(viewData);
            mocks.VerifyAll();
        }

        [Test]
        public void GetDataOfActiveView_HasActiveView_ReturnViewData()
        {
            // Setup
            ...
            var viewsList = mocks.Stub<IViewList>();
            viewsList.ActiveView = activeView;
            var documentViewController = mocks.Stub<IDocumentViewController>();
            documentViewController.Stub(c => c.DocumentViews).Return(viewsList);
            var toolViewController = mocks.Stub<IToolViewController>();
            var applicationSelection = mocks.Stub<IApplicationSelection>();
            ...
            // Call
            var returnedViewData = commandHandler.GetDataOfActiveView();

            // Assert
            Assert.AreSame(viewData, returnedViewData);
            mocks.VerifyAll();
        }

        [Test]
        public void CanOpenSelectViewDialog_NoSelection_ReturnFalse()
        {
            // Setup
            var mocks = new MockRepository();
            var documentViewController = mocks.Stub<IDocumentViewController>();
            ...
            // Call
            var result = commandHandler.CanOpenSelectViewDialog();
        ...
        [TestCase(0)]
        [TestCase(1)]
        public void CanOpenSelectViewDialog_LessThenTwoViews_ReturnFalse(int numberOfViewsForSelection)
        {
            // Setup
            var viewInfos = new ViewInfo[numberOfViewsForSelection];
            ...
            viewResolver.Expect(r => r.GetViewInfosFor(selectedObject)).Return(viewInfos);
            var documentViewController = mocks.Stub<IDocumentViewController>();
            documentViewController.Expect(c => c.DocumentViewsResolver).Return(viewResolver);
            var toolViewController = mocks.Stub<IToolViewController>();
            var applicationSelection = mocks.Stub<IApplicationSelection>();
        ...
        [TestCase(2)]
        [TestCase(12)]
        public void CanOpenSelectViewDialog_MoreThenOneView_ReturnTrue(int numberOfViewsForSelection)
        {
            // Setup
            var viewInfos = new ViewInfo[numberOfViewsForSelection];
            for (int i = 0; i < viewInfos.Length; i++)
        ...
        [Test]
        public void OpenSelectViewDialog_ForceOpenViewDialogForSelection()
        {
            // Setup
            var selectedObject = new object();
            ...
            var mocks = new MockRepository();
            var viewResolver = mocks.Stub<IViewResolver>();
            viewResolver.Expect(r => r.OpenViewForData(selectedObject, true)).Return(true);
            var documentViewController = mocks.Stub<IDocumentViewController>();
            ...
            // Call
            commandHandler.OpenSelectViewDialog();
        ...
        [TestCase(1)]
        [TestCase(11)]
        public void CanOpenViewFor_NoViewInfosForTarget_ReturnFalse(int numberOfViewDefinitions)
        {
            // Setup
            var viewObject = new object();
