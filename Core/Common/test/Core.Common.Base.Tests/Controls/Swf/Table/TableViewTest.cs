...
namespace Core.Common.Base.Tests.Controls.Swf.Table
{
    ...
    public class TableViewTest
    {
        ...
        public void TableViewCursorKeysWhileInCellEditModeShouldWorkLikeExcel()
        {
            ...
            WindowsFormsTestHelper.ShowModal(tableView,
                                             f =>
                                             {
                                                 ...
                                                 cell = TypeUtils.CallPrivateMethod<TableViewCell>(tableView, "GetFocusedCell");
                                                 Assert.AreEqual(0, cell.RowIndex);
                                                 Assert.AreEqual(0, cell.Column.DisplayIndex);
                                             });
        }
        ...
        public void ColumnVisibilityPropertyShouldNotAffectDisplayIndexTools()
        {
            ...
            tableView.AddColumn("propertyName2", "Property name 2");

            var propertyName1Column = tableView.Columns[0];
            var propertyName2Column = tableView.Columns[1];

            Assert.AreEqual(0, propertyName1Column.AbsoluteIndex);
            Assert.AreEqual(1, propertyName2Column.AbsoluteIndex);
            Assert.AreEqual(0, propertyName1Column.DisplayIndex);
            Assert.AreEqual(1, propertyName2Column.DisplayIndex);

            propertyName2Column.Visible = false;
            ...
            propertyName2Column.Visible = true;

            Assert.AreEqual(0, propertyName1Column.AbsoluteIndex);
            Assert.AreEqual(1, propertyName2Column.AbsoluteIndex);
            Assert.AreEqual(0, propertyName1Column.DisplayIndex, "Display indexes are expected to be identical to starting situation.");
            Assert.AreEqual(1, propertyName2Column.DisplayIndex, "Display indexes are expected to be identical to starting situation.");
        }
