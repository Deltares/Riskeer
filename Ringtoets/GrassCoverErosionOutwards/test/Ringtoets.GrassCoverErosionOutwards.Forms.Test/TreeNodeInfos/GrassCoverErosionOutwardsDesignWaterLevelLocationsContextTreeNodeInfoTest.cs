            ...
            using (var treeViewControl = new TreeViewControl())
            {
                ...
                using (var plugin = new GrassCoverErosionOutwardsPlugin())
                {
                    ...
                    using (ContextMenuStrip contextMenuAdapter = info.ContextMenuStrip(context, null, treeViewControl))
                    {
                        ...
                        // Then
                        string message = string.Format("Berekeningen konden niet worden gestart. Fout bij het lezen van bestand '{0}': het bestand bestaat niet.", filePath);
                ...
                using (var plugin = new GrassCoverErosionOutwardsPlugin())
                {
                    ...
                    using (new HydraRingCalculatorFactoryConfig())
                    {
                        ...
                        TestHelper.AssertLogMessages(call, messages =>
                        {
                            ...
                            StringAssert.StartsWith(string.Format("Validatie van 'Waterstand bij doorsnede-eis voor locatie '{0}'' gestart om: ", hydraulicBoundaryLocation.Name), msgs[0]);
                            StringAssert.StartsWith(string.Format("Validatie van 'Waterstand bij doorsnede-eis voor locatie '{0}'' beëindigd om: ", hydraulicBoundaryLocation.Name), msgs[1]);
                            StringAssert.StartsWith(string.Format("Berekening van 'Waterstand bij doorsnede-eis voor locatie '{0}'' gestart om: ", hydraulicBoundaryLocation.Name), msgs[2]);
                            Assert.AreEqual(string.Format("Waterstand bij doorsnede-eis berekening voor locatie '{0}' is niet geconvergeerd.", hydraulicBoundaryLocation.Name), msgs[3]);
                            StringAssert.StartsWith("Toetspeil berekening is uitgevoerd op de tijdelijke locatie",
                                                    msgs[4]);
                            StringAssert.StartsWith(string.Format("Berekening van 'Waterstand bij doorsnede-eis voor locatie '{0}'' beëindigd om: ", hydraulicBoundaryLocation.Name), msgs[5]);
                            StringAssert.AreNotEqualIgnoringCase(string.Format("Uitvoeren van '{0}' is gelukt.", hydraulicBoundaryLocation.Name), msgs[6]);
                        });
