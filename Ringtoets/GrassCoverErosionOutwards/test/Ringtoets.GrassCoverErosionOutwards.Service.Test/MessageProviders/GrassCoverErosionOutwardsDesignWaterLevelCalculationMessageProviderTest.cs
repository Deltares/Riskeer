...
namespace Ringtoets.GrassCoverErosionOutwards.Service.Test.MessageProviders
{
    ...
    public class GrassCoverErosionOutwardsDesignWaterLevelCalculationMessageProviderTest
    {
        ...
        public void GetCalculationName_ValidNames_ExpectedValue(string name)
        {
            ...
            // Assert
            string expectedName = string.Format("Waterstand bij doorsnede-eis voor locatie '{0}'", name);
        ...
        public void GetActivityName_ValidNames_ExpectedValue(string name)
        {
            ...
            // Assert
            string expectedName = string.Format("Waterstand bij doorsnede-eis berekenen voor locatie '{0}'", name);
        ...
        public void GetCalculationFailedMessage_ValidNames_ExpectedValue(string name)
        {
            ...
            // Assert
            string expectedMessage = $"Er is een fout opgetreden tijdens de Waterstand bij doorsnede-eis berekening '{name}'. Bekijk het foutrapport door op details te klikken.{Environment.NewLine}{failureMessage}";
        ...
        public void GetCalculatedNotConvergedMessage_ValidNames_ExpectedValue(string name)
        {
            ...
            // Assert
            string expectedMessage = string.Format("Waterstand bij doorsnede-eis berekening voor locatie '{0}' is niet geconvergeerd.", name);
            Assert.AreEqual(expectedMessage, message);
        }
        ...
        public void GetCalculationFailedUnexplainedMessage_ValidNames_ExpectedValues(string name)
        {
            ...
            // Assert
            string expectedMessage = string.Format("Er is een fout opgetreden tijdens de Waterstand bij doorsnede-eis berekening '{0}'. Er is geen foutrapport beschikbaar", name);
            Assert.AreEqual(expectedMessage, message);
        }
    }
}