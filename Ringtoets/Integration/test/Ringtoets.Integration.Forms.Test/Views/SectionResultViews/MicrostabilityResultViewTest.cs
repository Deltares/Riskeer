...
namespace Ringtoets.Integration.Forms.Test.Views.SectionResultViews
{
    ...
    public class MicrostabilityResultViewTest
    {
        ...
        {
            // Setup
            var failureMechanismSectionResults = new ObservableList<MicrostabilityFailureMechanismSectionResult>();

            // Call
            using (var view = new MicrostabilityResultView(failureMechanismSectionResults))
            {
                // Assert
                Assert.IsInstanceOf<FailureMechanismResultView<MicrostabilityFailureMechanismSectionResult>>(view);
        ...
        public void GivenFormWithFailureMechanismResultView_WhenDataSourceWithFailureMechanismSectionResultAssigned_ThenSectionsAddedAsRows()
        {
            ...
            var sectionResults = new ObservableList<MicrostabilityFailureMechanismSectionResult>
            {
                result1,
                result2,
                result3
            ...
            using (var form = new Form())
            using (var view = new MicrostabilityResultView(sectionResults))
            {
                form.Controls.Add(view);
                form.Show();
