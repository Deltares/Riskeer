    ...
    public class GrassCoverErosionInwardsCalculationServiceTest
    {
        ...
        public void Calculate_OvertoppingRateCalculationFails_OutputNotNull(OvertoppingRateCalculationType overtoppingRateCalculationType)
        {
            ...
            IAssessmentSection assessmentSectionStub = AssessmentSectionHelper.CreateAssessmentSectionStub(failureMechanism,
                                                                                                           mockRepository,
                                                                                                           validFile);
            var testOvertoppingRateCalculator = new TestHydraulicLoadsCalculator
            {
                EndInFailure = true
            };
            var calculatorFactory = mockRepository.Stub<IHydraRingCalculatorFactory>();
            ...
            TestHelper.AssertLogMessages(call, messages =>
            {
                ...
                Assert.AreEqual($"Berekening van '{calculation.Name}' gestart.", msgs[0]);
                StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[1]);
                StringAssert.StartsWith($"De overslagdebiet berekening voor grasbekleding erosie kruin en binnentalud '{calculation.Name}' is niet gelukt.", msgs[2]);
        ...
        public void Calculate_CancelWithValidOvertoppingCalculationInput_CancelsCalculatorAndHasNullOutput()
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                testOvertoppingCalculator.CalculationFinishedHandler += (s, e) => service.Cancel();
                ...
                // Assert
                Assert.IsTrue(testOvertoppingCalculator.IsCanceled);
                ...
                DikeHeightCalculationType.CalculateByProfileSpecificRequiredProbability)] DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                ...
                {
                    testDikeHeightCalculator.CalculationFinishedHandler += (s, e) => service.Cancel();
                }
                ...
                Assert.IsNull(calculation.Output);
                Assert.IsTrue(testOvertoppingCalculator.IsCanceled);
                Assert.IsTrue(testDikeHeightCalculator.IsCanceled);
            }
        }
                ...
                OvertoppingRateCalculationType.CalculateByProfileSpecificRequiredProbability)] OvertoppingRateCalculationType overtoppingRateCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                TestOvertoppingCalculator overToppingCalculator = testOvertoppingCalculator;
        ...
        public void Calculate_OvertoppingCalculationFailedWithExceptionAndNoLastErrorPresent_LogErrorAndThrowException()
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    Assert.AreEqual(4, msgs.Length);
                    Assert.AreEqual($"Berekening van '{calculation.Name}' gestart.", msgs[0]);
                    Assert.AreEqual($"De overloop en overslag berekening voor grasbekleding erosie kruin en binnentalud '{calculation.Name}' is niet gelukt. Er is geen foutrapport beschikbaar.", msgs[1]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[2]);
                    Assert.AreEqual($"Berekening van '{calculation.Name}' beëindigd.", msgs[3]);
                });
                Assert.IsTrue(exceptionThrown);
        ...
        public void Calculate_DikeHeightCalculationFailedWithExceptionAndNoLastErrorPresent_LogError(DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    Assert.AreEqual($"Berekening van '{calculation.Name}' gestart.", msgs[0]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[1]);
                    Assert.AreEqual($"De HBN berekening voor grasbekleding erosie kruin en binnentalud '{calculation.Name}' is niet gelukt. Er is geen foutrapport beschikbaar.", msgs[2]);
                    StringAssert.StartsWith("De HBN berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    Assert.AreEqual($"Berekening van '{calculation.Name}' beëindigd.", msgs[4]);
                });
                Assert.IsNotNull(calculation.Output);
        ...
        public void Calculate_OvertoppingRateCalculationFailedWithExceptionAndNoLastErrorPresent_LogError(OvertoppingRateCalculationType overtoppingRateCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    Assert.AreEqual($"Berekening van '{calculation.Name}' gestart.", msgs[0]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[1]);
                    Assert.AreEqual($"De overslagdebiet berekening voor grasbekleding erosie kruin en binnentalud '{calculation.Name}' is niet gelukt. Er is geen foutrapport beschikbaar.", msgs[2]);
                    StringAssert.StartsWith("De overslagdebiet berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    Assert.AreEqual($"Berekening van '{calculation.Name}' beëindigd.", msgs[4]);
                });
                Assert.IsNotNull(calculation.Output);
