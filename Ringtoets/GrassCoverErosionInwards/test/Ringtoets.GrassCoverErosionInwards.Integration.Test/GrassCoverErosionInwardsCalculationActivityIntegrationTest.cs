            ...
            DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig())
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' gestart om: ", calculation.Name), msgs[2]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    StringAssert.StartsWith(string.Format("De dijkhoogte berekening voor grasbekleding erosie kruin en binnentalud '{0}' is niet gelukt.", calculation.Name), msgs[4]);
                    StringAssert.StartsWith("De dijkhoogte berekening is uitgevoerd op de tijdelijke locatie", msgs[5]);
        ...
        public void Run_CalculateWithDikeHeightAndRan_PerformValidationAndCalculationAndLogStartAndEndError(DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig())
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' gestart om: ", calculation.Name), msgs[2]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    StringAssert.StartsWith("De dijkhoogte berekening is uitgevoerd op de tijdelijke locatie", msgs[4]);
                    StringAssert.StartsWith(string.Format("De dijkhoogte berekening voor grasbekleding erosie kruin en binnentalud '{0}' is niet geconvergeerd.", calculation.Name), msgs[5]);
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' beëindigd om: ", calculation.Name), msgs[6]);
                });
                Assert.AreEqual(ActivityState.Executed, activity.State);
            }
        ...
        public void Run_DikeHeightCalculationFailedWithExceptionAndLastErrorPresent_LogError(DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig())
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' gestart om: ", calculation.Name), msgs[2]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    StringAssert.StartsWith(string.Format("De dijkhoogte berekening voor grasbekleding erosie kruin en binnentalud '{0}' is niet gelukt. Bekijk het foutrapport door op details te klikken.", calculation.Name), msgs[4]);
                    StringAssert.StartsWith("De dijkhoogte berekening is uitgevoerd op de tijdelijke locatie", msgs[5]);
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' beëindigd om: ", calculation.Name), msgs[6]);
                });
                Assert.AreEqual(ActivityState.Executed, activity.State);
        ...
        public void Run_DikeHeightCalculationFailedWithExceptionAndNoLastErrorPresent_LogError(DikeHeightCalculationType dikeHeightCalculationType)
        {
            ...
            using (new HydraRingCalculatorFactoryConfig())
            {
                ...
                TestHelper.AssertLogMessages(call, messages =>
                {
                    ...
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' gestart om: ", calculation.Name), msgs[2]);
                    StringAssert.StartsWith("De overloop en overslag berekening is uitgevoerd op de tijdelijke locatie", msgs[3]);
                    StringAssert.StartsWith(string.Format("De dijkhoogte berekening voor grasbekleding erosie kruin en binnentalud '{0}' is niet gelukt. Er is geen foutrapport beschikbaar.", calculation.Name), msgs[4]);
                    StringAssert.StartsWith("De dijkhoogte berekening is uitgevoerd op de tijdelijke locatie", msgs[5]);
                    StringAssert.StartsWith(string.Format("Berekening van '{0}' beëindigd om: ", calculation.Name), msgs[6]);
                });
                Assert.AreEqual(ActivityState.Executed, activity.State);
