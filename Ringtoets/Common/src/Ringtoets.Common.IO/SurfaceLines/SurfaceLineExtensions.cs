...
namespace Ringtoets.Common.IO.SurfaceLines
{
    ...
    public static class SurfaceLineExtensions
    {
        ...
        {
            IEnumerable<Segment2D> surfaceLineSegments = Math2D.ConvertPointsToLineSegments(surfaceLine.Points.Select(p => new Point2D(p.X, p.Y)));
            Segment2D[] referenceLineSegments = Math2D.ConvertPointsToLineSegments(referenceLine.Points).ToArray();
