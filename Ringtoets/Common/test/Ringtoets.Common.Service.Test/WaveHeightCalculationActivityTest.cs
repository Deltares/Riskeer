            ...
            using (new HydraRingCalculatorFactoryConfig(calculatorFactory))
            {
                ...
                TestHelper.AssertLogMessages(call, m =>
                {
                    ...
                    string expectedFailureMessage = string.IsNullOrEmpty(lastErrorFileContent)
                                                        ? $"Er is een fout opgetreden tijdens de golfhoogte berekening voor locatie '{locationName}' (Categorie {categoryBoundaryName}). Er is geen foutrapport beschikbaar."
                                                        : $"Er is een fout opgetreden tijdens de golfhoogte berekening voor locatie '{locationName}' (Categorie {categoryBoundaryName}). Bekijk het foutrapport door op details te klikken.{Environment.NewLine}{lastErrorFileContent}";
