...
{
    [TestFixture]
    public class SemiProbabilisticDesignValueFactoryTest
    {
        [Test]
        public void CreateDesignVariable_WithDistribution_Return()
        {
            // Setup
            ...
            VariationCoefficientDesignVariable<VariationCoefficientLogNormalDistribution> designVariable = 
                SemiProbabilisticDesignValueFactory.CreateDesignVariable(distribution, 0.05);

            // Assert
            Assert.AreSame(distribution, designVariable.Distribution);
            Assert.AreEqual(0.1, designVariable.GetDesignValue());
        }
    }
}